...
// Sliding TTL (ms) via .env SESSION_TIMEOUT; default 30 min
...
function randomToken(bytes = 48) {
  // Compact, URL-safe random token
  return crypto.randomBytes(bytes).toString('base64url');
}

/**
 * Create a session for a user.
 * If singleSession=true, invalidate other active sessions for this user.
 */
async function createSession({ userId, ip, ua, singleSession = true }) {
  ...
  try {
    ...
    return rows[0]; // { session_id, token, user_id, expires_at }
  } catch (e) {
...
/**
 * Verify session token (active + not expired).
 */
...
/**
 * Extend session expiration (sliding TTL).
 */
...
/**
 * Revoke a session by token or by numeric session_id.
 */
