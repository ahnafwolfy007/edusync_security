...
class AuthController {
  ...
  // User login
  async login(req, res) {
    // Brute-force guard context (set by middleware)
    ...
    try {
      ...
      // NEW: Create DB session + set cookie + return session token
      const sess = await createSession({
        userId: user.user_id,
        ip: req.ip,
        ua: req.headers['user-agent'],
        singleSession: true // set false to allow multiple active sessions per user
      });
  ...
  async logout(req, res) {
    try {
      // JWT blacklist (if Bearer provided)
      ...
      // Revoke DB session (if Session header or cookie provided)
  ...
  async changePassword(req, res) {
    try {
      ...
      // Verify current password (uses your custom scheme helpers)
      let currentValid = false;
      if (user.password_hash) {
        currentValid = verifyPasswordHash(currentPassword, user.password_hash, (req.user?.email || '').toLowerCase());
      }
      if (!currentValid) {
        console.warn('[ChangePassword] Current password verification failed.');
      }
      if (!currentValid) {
        return res.status(400).json({
          success: false,
      ...
      // Hash new password with email-based salt
