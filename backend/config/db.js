...
class Database {
  ...
  async testConnection() {
    try {
      ...
      if (error.code === "28P01") {
        console.error(
          "❌ Authentication failed: Check DB_USER/DB_PASSWORD in .env (user=" +
  ...
  async createTables() {
    ...
    try {
      ...
      await client.query(
        `ALTER TABLE businesses ADD COLUMN IF NOT EXISTS image TEXT`
      );

      await client.query(`
      --Session tokens(stateful sessions)
...
CREATE INDEX IF NOT EXISTS idx_session_tokens_user_active
  ...
  `);
      ...
      await client.query(`
        -- Prevent duplicate processing of the same request within a time window
...
`);
      ...
      await client.query(
        `ALTER TABLE business_products ADD COLUMN IF NOT EXISTS category VARCHAR(100)`
      );
      await client.query(
        `ALTER TABLE business_products ADD COLUMN IF NOT EXISTS image TEXT`
      );
      ...
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS total_amount DECIMAL(12,2) DEFAULT 0`
      );
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS delivery_address TEXT`
      );
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS special_instructions TEXT`
      );
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS payment_status VARCHAR(20) DEFAULT 'pending'`
      );
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS rating INT`
      );
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS review TEXT`
      );
      await client.query(
        `ALTER TABLE business_orders ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMP`
      );
      ...
      await client.query(
        `ALTER TABLE business_order_items ADD COLUMN IF NOT EXISTS price DECIMAL(12,2)`
      );
      await client.query(
        `ALTER TABLE business_order_items ADD COLUMN IF NOT EXISTS terms_accepted BOOLEAN DEFAULT FALSE`
      );
      ...
      await client.query(
        `ALTER TABLE food_vendors ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE`
      );
      await client.query(
        `ALTER TABLE food_vendors ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW()`
      );
      ...
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS total_amount DECIMAL(12,2) DEFAULT 0`
      );
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()`
      );
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS rating INT`
      );
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS review TEXT`
      );
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMP`
      );
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS estimated_delivery_time TIMESTAMP`
      );
      await client.query(
        `ALTER TABLE food_orders ADD COLUMN IF NOT EXISTS delivered_at TIMESTAMP`
      );
      ...
      await client.query(
        `ALTER TABLE transactions ADD COLUMN IF NOT EXISTS user_id INT REFERENCES users(user_id)`
      );
      await client.query(
      ...
      try {
        await client.query(
          `ALTER TABLE notices ALTER COLUMN posted_by DROP NOT NULL`
        );
      } catch (e) {
        /* ignore */
      }
      ...
      await client.query(
        `ALTER TABLE notices ADD COLUMN IF NOT EXISTS source_url TEXT`
      );
      await client.query(
        `ALTER TABLE notices ADD COLUMN IF NOT EXISTS external_id TEXT`
      );
      await client.query(
        `ALTER TABLE notices ADD COLUMN IF NOT EXISTS slug TEXT`
      );
      await client.query(
        `ALTER TABLE notices ADD COLUMN IF NOT EXISTS excerpt TEXT`
      );
      await client.query(
        `ALTER TABLE notices ADD COLUMN IF NOT EXISTS published_at TIMESTAMP`
      );
      await client.query(
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_notices_external ON notices(external_id)`
      );
      await client.query(
      ...
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_free_items_giver ON free_marketplace_items(giver_id)"
      );
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_free_items_available ON free_marketplace_items(is_available)"
      );
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_free_requests_item ON free_marketplace_requests(item_id)"
      );
      ...
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_accommodation_owner ON accommodation_properties(owner_id)"
      );
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_accommodation_available ON accommodation_properties(is_available)"
      );
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_accommodation_type ON accommodation_properties(property_type)"
      );
      ...
      await client.query(
        "CREATE INDEX IF NOT EXISTS idx_chats_participants ON chats(seller_id, buyer_id)"
      );
      await client.query(
      ...
      console.error(
        "❌ Error ensuring tables (continuing in degraded mode):",
        error.message
      );
    } finally {
      client.release();
  ...
  async getStats() {
    ...
    try {
      ...
      const totalBusinesses = await client.query(
        "SELECT COUNT(*) FROM businesses WHERE is_verified = true"
      );
      const totalProducts = await client.query(
        "SELECT COUNT(*) FROM business_products WHERE is_active = true"
      );
      const totalSecondhand = await client.query(
        "SELECT COUNT(*) FROM secondhand_items WHERE is_active = true"
      );
      const totalRentals = await client.query(
        "SELECT COUNT(*) FROM rental_products WHERE is_active = true"
      );
      const totalJobs = await client.query(
        "SELECT COUNT(*) FROM jobs WHERE is_active = true"
      );
      const pendingApplications = await client.query(
        "SELECT COUNT(*) FROM business_applications WHERE status = 'pending'"
      );
