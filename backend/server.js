...
app.use('/api', security.securityMiddleware()); // Concurrent request checking
app.use('/api', security.actionMonitor()); // Action monitoring
app.use('/api', security.contentMonitor()); // Content spam detection
...
// Security routes with middleware injection
app.use('/api/security', (req, res, next) => {
  req.securityInstance = security;
  next();
}, securityRoutes);

// API Routes
app.use('/api/auth', security.loginMonitor(), authRoutes); // Add login monitoring
...
app.use('/api/admin', security.adminSecurityMiddleware(), adminRoutes);
